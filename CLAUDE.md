...
- **Card payment fee**: Configurable percentage (default 5%, stored in `app_settings` as `card_fee_percent`) of order total, rounded up to nearest 10 baht (`calcCardFee` in `format.ts`). Owner can adjust fee % directly in `CheckoutDialog`. Passed as `extraAmount` through `useSale.checkout()` → `sales:create` IPC. Receipt shows a `ค่าบริการชำระบัตร` line when fee is present (derived from `total_amount - sum(sale_items)`).
...
- **Input validation**: IPC handlers validate inputs server-side — product name non-empty, prices/stock >= 0, seller_role enum, JSON validity for parked orders, store name non-empty + unique constraint handling. Frontend forms have inline validation with real-time error messages and duplicate name detection for products/customers.
- **Settings**: Generic key-value settings via `settings:get` / `settings:set` IPC, stored in `app_settings` table. Used for `card_fee_percent`, `owner_password`, etc.
- **CSV Export**: `export:*` IPC handlers generate CSV files with UTF-8 BOM for Thai character support in Excel. Available exports: sales summary, sales detail (per-item), products, expenses, customers with debt. Export buttons on History, Stock, Expense, and Customer pages.
...
- **Pagination**: Shared `Pagination` component supports optional page size selector (10/20/50/100). Used in History and Expense pages.
